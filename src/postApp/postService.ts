...
async function createPost(
	...
	try {
		...
		return {
			status: "error",
			message:
				err instanceof Error ? err.message : "Не вдалося створити пост",
		};
	}
}
		...
		if (data.tags && Array.isArray(data.tags)) {
			...
			if (validTags.length !== data.tags.length) {
				...
				return {
					status: "error",
					message:
						"Некоректні теги або занадто довгі (макс. 50 символів)",
				};
			}
			...
			const imagesToDelete = currentImages.filter((currentImg) => {
				if (data.images)
					return data.images.some(
						(newImg) => newImg.id === currentImg.image.id,
					);
			});
