		...
		s = (a) => {
			a.forEach((f) => {
				(f.includes("Uint") && (t[f.replace("Uint", "UInt")] = t[f]),
					f.includes("Float64") &&
						(t[f.replace("Float64", "Double")] = t[f]),
					f.includes("Float32") &&
						(t[f.replace("Float32", "Float")] = t[f]));
			});
		};
	n.forEach((a, f) => {
		(a.startsWith("read") &&
...
function bn(t, e) {
	...
	if (
		...
		for (
			i = f.length, a = C.length, T = i < a, T && (a = i), i = 0;
			i < a;
			i++
		)
			if (f[i] != C[i]) {
...
var Fe,
	...
	xn = ne(() => {
		...
		R.comparedTo = R.cmp = function (t) {
			...
			for (
				n = o.d.length, i = t.d.length, e = 0, r = n < i ? n : i;
				e < r;
				++e
			)
				if (o.d[e] !== t.d[e])
		...
		R.toPrecision = function (t, e) {
			...
			return (
				t === void 0
					? ((r = $(i)),
						(n = Me(i, r <= o.toExpNeg || r >= o.toExpPos)))
					: (ce(t, 1, Fe),
						e === void 0 ? (e = o.rounding) : ce(e, 0, 8),
						(i = _(new o(i), t, e)),
						(r = $(i)),
	...
	if (
		(n
			? (a.push(
				...
				a.push(
					s.red(
						`It occurred in the ${s.bold(`\`${t}\``)} invocation${f}`,
					),
				))
...
var Ge = class extends Te {
	...
	writeEmpty(e) {
		let r = new de("[]");
		(this.hasError && r.setColor(e.context.colors.red).underline(),
			e.write(r));
	}
	writeWithItems(e) {
	...
	writeWithContents(e, r) {
		(e.writeLine("{").withIndent(() => {
			...
			this.hasError &&
				e.afterNextNewline(() => {
					e.writeLine(
						e.context.colors.red("~".repeat(this.getPrintWidth())),
					);
				}));
	}
};
				...
				? t.constraints.requiredFields
					? o.push(
							`${i.green("at least one of")} ${Jt(
								"or",
								t.constraints.requiredFields.map(
									(s) => `\`${i.bold(s)}\``,
								),
							)} arguments.`,
						)
		...
		r = (n, i) =>
			e.getOrCreate(n, () =>
				i.has(n)
					? [n]
					: (i.add(n),
						t[n] ? t[n].needs.flatMap((o) => r(o, i)) : [n]),
			);
			...
			return this.queryCallbacksCache.getOrCreate(`${e}:${r}`, () => {
				...
				return !o || !(o[e] || o.$allModels || o[r] || o.$allOperations)
					...
					: (o[e] !== void 0 &&
							...
							o[e].$allOperations !== void 0 &&
								i.push(o[e].$allOperations)),
						e !== "$none" &&
							o.$allModels !== void 0 &&
...
var Z = class t {
	constructor(e, r) {
		...
		let n = r.reduce(
			(s, a) => s + (a instanceof t ? a.values.length : 1),
			0,
		);
...
function lr(t = {}) {
	return (e) => (
		typeof t._count == "boolean" && (e._count = e._count._all),
		e
	);
}
function Vi(t, e) {
...
var en = (t, e = new Zr()) => {
		let r = {
			...
			startTransaction: async (...n) =>
				(await Ee(e, t.startTransaction.bind(t))(...n)).map((o) =>
					hl(e, o),
				),
		};
		return (
...
var Rt = class {
	...
	wrapEngine(e) {
		return {
			...
			rollbackTransaction: this.withRequestId(
				e.rollbackTransaction.bind(e),
			),
	...
	async loadEngine() {
		if (!this.engine) {
			...
			try {
				let e = new w(this);
				this.adapterPromise ||
					(this.adapterPromise = this.config.adapter
						?.connect()
						?.then(en));
...
${a.backtrace}`,
	...
	getExternalAdapterError(e, r) {
		if (e.error_code === wl && r) {
			...
			return (
				qt(i, "External error with reported id was not registered"),
				i
			);
		}
	}
	...
	pr = class {
		constructor(e) {
			...
			((this.resolveDatasourceUrl =
				...
				(this.engineVersion =
					this.config.accelerateUtils?.engineVersion),
				(this.clientVersion =
					this.config.accelerateUtils?.clientVersion));
		}
		...
		(r) => {
			...
			if (rr(r))
				...
				switch (e) {
					...
					default:
						throw new Error(
							`The ${e} provider does not support ${t}`,
						);
				}
			return (
				i?.values
...
function Il(t) {
	if (t) {
		if (t.kind === "batch")
			return {
				kind: "batch",
				options: { isolationLevel: t.isolationLevel },
			};
...
function Go(t) {
	class e {
		...
		constructor(n) {
			((t = n?.__internal?.configOverride?.(t) ?? t),
				mo(t),
				n && Vo(n, t));
				...
				if (
					...
					(this._engineConfig = {
						...
						transactionOptions: {
							...
							isolationLevel:
								f.transactionOptions?.isolationLevel,
						},
						logEmitter: i,
